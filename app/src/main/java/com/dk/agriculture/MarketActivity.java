...
public class MarketActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FirebaseDatabase.getInstance().getReference("market").child(getIntent().getStringExtra("equipment")).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                for(DataSnapshot fire : dataSnapshot.getChildren())
                {
                    MarketAddModel temp = fire.getValue(MarketAddModel.class);
                    if(!temp.getRentername().matches(db.getAllValues().get("name"))){
                    sellerlist.add(temp);
                    }
                }
                putDistance(sellerlist,getIntent().getDoubleExtra("lat",0),getIntent().getDoubleExtra("lon",0));
